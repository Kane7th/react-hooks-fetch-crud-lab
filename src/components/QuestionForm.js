...
function QuestionForm({ onQuestionSubmit }) {
  // Changed prop name from setQuestions to onQuestionSubmit
  ...
  useEffect(() => {
    return () => {
      setIsMounted(false); // Clean up when the component is unmounted
    };
  }, []);
  ...
  function handleSubmit(event) {
    ...
    fetch("http://localhost:4000/questions", {
      ...
      .then((data) => {
        if (onQuestionSubmit) {
          onQuestionSubmit(data); // Changed from setQuestions to onQuestionSubmit
        }
        setFormData({
          prompt: "",
          answer1: "",
